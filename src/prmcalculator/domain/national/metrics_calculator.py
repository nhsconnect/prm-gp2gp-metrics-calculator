...
def _count_failed_transfers(transfers: List[Transfer]) -> int:
    return len(
        [t for t in transfers if t.outcome.failure_reason == TransferFailureReason.FINAL_ERROR]
    )


def _count_transfers_with_statuses(transfers: List[Transfer], statuses: Set[TransferStatus]) -> int:
    return len([t for t in transfers if t.outcome.status in statuses])


def _count_pending_transfers(transfers: List[Transfer]):
    count = 0
...
def _filter_for_integrated_transfers(transfers: List[Transfer]) -> List[Transfer]:
    return [
        t
        for t in transfers
        if (
