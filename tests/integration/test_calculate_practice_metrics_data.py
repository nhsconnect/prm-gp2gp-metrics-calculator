...
def test_calculates_correct_practice_metrics_given_a_successful_transfer():
    ...
    expected_integrated_metrics = IntegratedPracticeMetricsPresentation(
        transfer_count=1,
                ...
                metrics=[
                    MonthlyMetricsPresentation(
                        ...
                        requester=RequesterMetrics(
                            integrated=expected_integrated_metrics,
                            transfers_received=TransfersReceivedPresentation(
                                transfer_count=1,
                                awaiting_integration=AwaitingIntegration(percentage=0),
                                integrated=expected_integrated_metrics,
                            ),
                        ),
                    )
